...
import com.gpl.rpg.GotandaRPG.activity.DisplayWorldMapActivity;
import com.gpl.rpg.GotandaRPG.context.WorldContext;
import com.gpl.rpg.GotandaRPG.model.map.*;
import com.gpl.rpg.GotandaRPG.model.map.WorldMapSegment.NamedWorldMapArea;
import com.gpl.rpg.GotandaRPG.model.map.WorldMapSegment.WorldMapSegmentMap;
	...
	private static String getWorldMapSegmentAsHtml(Resources res, WorldContext world, String segmentName) {
		...
		for (WorldMapSegmentMap segmentMap : segment.maps.values()) {
			...
			mapsAsHtml
				...
				.append("px;\" />");
			if (GotandaRPGApplication.DEVELOPMENT_DEBUGMESSAGES) mapsAsHtml.append('\n');
		...
		for (NamedWorldMapArea area : segment.namedAreas.values()) {
			...
			namedAreasAsHtml
				...
				.append(area.name)
				.append("</span></div>");
			if (GotandaRPGApplication.DEVELOPMENT_DEBUGMESSAGES) namedAreasAsHtml.append('\n');
		}
