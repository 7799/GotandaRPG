package com.gpl.rpg.GotandaRPG.controller;
...
import com.gpl.rpg.GotandaRPG.controller.listeners.PlayerMovementListeners;
...
import com.gpl.rpg.GotandaRPG.model.map.MonsterSpawnArea;
...
import com.gpl.rpg.GotandaRPG.util.TimedMessageTask;
...
public final class MovementController implements TimedMessageTask.Callback {
	...
	private boolean findWalkablePosition(int dx, int dy) {
		...
		if (controllers.preferences.movementAggressiveness == GotandaRPGPreferences.MOVEMENTAGGRESSIVENESS_NORMAL) return false;

		return findWalkablePosition(dx, dy, GotandaRPGPreferences.MOVEMENTAGGRESSIVENESS_NORMAL);
	}

	public boolean findWalkablePosition(int dx, int dy, int aggressiveness) {
		if (controllers.preferences.movementMethod == GotandaRPGPreferences.MOVEMENTMETHOD_STRAIGHT) {
	...
	private boolean tryWalkablePosition(int dx, int dy, int aggressiveness) {
		...
		if (aggressiveness == GotandaRPGPreferences.MOVEMENTAGGRESSIVENESS_NORMAL) return true;
		...
		if (aggressiveness == GotandaRPGPreferences.MOVEMENTAGGRESSIVENESS_AGGRESSIVE && m == null) return false;
		if (aggressiveness == GotandaRPGPreferences.MOVEMENTAGGRESSIVENESS_DEFENSIVE && m != null) return false;
