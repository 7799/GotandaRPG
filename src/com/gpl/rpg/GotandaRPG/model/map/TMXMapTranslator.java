...
import com.gpl.rpg.GotandaRPG.model.map.TMXMapFileParser.TMXLayer;
import com.gpl.rpg.GotandaRPG.model.map.TMXMapFileParser.TMXLayerMap;
import com.gpl.rpg.GotandaRPG.model.map.TMXMapFileParser.TMXMap;
import com.gpl.rpg.GotandaRPG.model.map.TMXMapFileParser.TMXObjectMap;
import com.gpl.rpg.GotandaRPG.model.map.TMXMapFileParser.TMXObject;
import com.gpl.rpg.GotandaRPG.model.map.TMXMapFileParser.TMXObjectGroup;
import com.gpl.rpg.GotandaRPG.model.map.TMXMapFileParser.TMXProperty;
import com.gpl.rpg.GotandaRPG.model.map.TMXMapFileParser.TMXTileSet;
import com.gpl.rpg.GotandaRPG.model.quest.QuestProgress;
import com.gpl.rpg.GotandaRPG.resource.tiles.TileCache;
import com.gpl.rpg.GotandaRPG.util.Coord;
import com.gpl.rpg.GotandaRPG.util.CoordRect;
import com.gpl.rpg.GotandaRPG.util.L;
import com.gpl.rpg.GotandaRPG.util.Range;
	...
	public ArrayList<PredefinedMap> transformMaps(Collection<TMXObjectMap> maps, MonsterTypeCollection monsterTypes, DropListCollection dropLists) {
		...
		for (TMXObjectMap m : maps) {
			...
			for (TMXProperty p : m.properties) {
				...
				else if(GotandaRPGApplication.DEVELOPMENT_VALIDATEDATA) L.log("OPTIMIZE: Map " + m.name + " has unrecognized property \"" + p.name + "\".");
			}
				...
				for (TMXObject object : group.objects) {
					...
					if (object.type == null) {
						...
						for (TMXProperty p : object.properties) {
							if (GotandaRPGApplication.DEVELOPMENT_VALIDATEDATA) L.log("OPTIMIZE: Map " + m.name + ", sign " + object.name + "@" + topLeft.toString() + " has unrecognized property \"" + p.name + "\".");
						}
						...
						for (TMXProperty p : object.properties) {
							...
							else if(GotandaRPGApplication.DEVELOPMENT_VALIDATEDATA) L.log("OPTIMIZE: Map " + m.name + ", mapchange " + object.name + "@" + topLeft.toString() + " has unrecognized property \"" + p.name + "\".");
						}
						...
						for (TMXProperty p : object.properties) {
							...
							} else if (GotandaRPGApplication.DEVELOPMENT_VALIDATEDATA) {
